...
We hypothesise, that this differential results might be caused by the naturally low amounts of LacI repressor present the in cells.
When we increase the copy number of \tax{lacZ} promoter, all LacI proteins becomes saturated and some promoters escape the repression, even though the plasmids used should produce only several copies within a cell \cite{zaslaver2006comprehensive}.
To confirm that we decided to place the \tax{lacZ::GFP} sequences into chromosome, creating only one additional \tax{lacZ} promoter per cell.
We have available pLW001 plasmid which is suitable for chromosomal integration (\hyperlink{pLW001}{Appendix 5}) in our lab made previously by a colleague Daying Wen.
...
Thus we decided to construct a new plasmid by replacing the GFP gene present in pLW001 vector with the GFP sequence we have in our negative control plasmid pUA66 (\hyperlink{pUA66seq}{Appendix 1}) together with its strong RBS, some additional restriction sites and terminator (Fig. \ref{cloning}).
\begin{figure}[h!]
  \centering
...
After cloning a colony PCR of using pUA66\textunderscore clon primers (Table \ref{pcr}) successfully amplified sequence of expected length (1100bp) in all 8 transformants tested (Fig. \ref{colonyPCR}).
PCR using ATT-RP2 and PSC-FP2 primers was then performed on clones 1:3(1) and 1:5(4) together with pLW001-Top10 and all these PCR products were sent for sequencing to Macrogen, Korea.
...
However, 4 SNPs compared to the expected reference were identified (Fig. \ref{1:3(1)seq}).
But as both sequenced clones have exactly the same SNPs (\hyperlink{pADOUCHseq}{Appendix 6}) and all of these SNPs were synonymous (Fig. \ref{1:3(1)seq}), it is probably that the sequence of GFP gene in the pUA66 plasmid we have is not accurate.
