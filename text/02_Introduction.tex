...
Even though some factors control only one promoter, the majority of at least \tax{E. coli} promoters is regulated by more of them \cite{karp2014ecocyc}.
...
To avoid wasteful production of enzyme hydrolysing lactose (LacZ), if no lactose is available, \tax{E. coli} expresses \tax{lac} repressor (LacI) \cite{hudson1990co}.
...
A negative feedback loop, i.e., a circuit where the product inhibits its own expression, exhibits the opposite traits compared to the positive feedback, as it decreases the response time \cite{rosenfeld2002negative} (Fig.~\ref{motifs}\textcolor{red}{b}).
...
If the level of CI reaches a certain threshold and outcompetes Cro, whose activity is suppressed, the phage enters a lysogenic cycle staying dormant in the cell thanks to the CI-\tax{cI} positive feedback loop.
...
Consistent with predictive behaviour \tax{E. coli} strongly represses genes responsible for aerobic metabolism to temperature increase (without a change in oxygen levels) in laboratory conditions \cite{tagkopoulos2008predictive}.
...
As the neutral signal caffeine was added to media which yeast \tax{S. cerevisiae} was expected to associate with the upcoming 5-fluoroorotic acid presence (metabolised into a toxic substance).
