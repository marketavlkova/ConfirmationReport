...
In that case the cell fate depends on the proteins or other small molecules it has already synthesised, and when none with an appropriate enzymatic activity is present, the cell has a higher chance of dying.
...
For the cell is thus more profitable to do not respond at once.
Cell might even benefit from not responding at all for some time, for example if the signal triggering costly response is only transient.
Transcriptional responses are also adequate in slowly changing environments (e.g. decrease in preferred carbon source concentration).
...
Physical signals such as UV or heat are also considered direct, although no active import takes place in the signalling, cell's interior cannot avoid them.
...
\subsubsection{Regulatory kinetics}
...
\textbf{Feedback loop} is a transcriptional network motif in which the product affects positively or negatively its own production  (Fig.~\ref{motifs}).
Such motifs have also its effect on the speed of cellular responses.
Negative feedback loop, i.e. circuit where the product inhibits its own expression, decreases the response time \cite{rosenfeld2002negative}.
...
\textbf{Feed forward loops} (FFLs) are also common regulatory motifs.
They affect response time in cells as well, but each type of FFL do so in a bit different way \cite{shen2002network}.
...
Coherent FFL type 1 (Fig.~\ref{motifs}\textcolor{red}{c}, C1), where both X and Y has to bind to trigger transcription (AND-gate logic) delays full gene expression compared to non-FFL system \cite{mangan2003coherent}, while no delay was observed in the inverse (ON to OFF) transition.
On the other hand, when only factor X or Y is sufficient to induce Z transcription (OR- gate logic in C1 FFL) transition from OFF to ON shows no delays  \cite{kalir2005coherent}.
But when factor X is deactivated the ON to OFF step is considerably delayed in comparison with strain with deleted gene for factor Y.
The former motif (AND-FFL) thus prevents Z expression when concentration of X fluctuates close to a threshold, while the latter circuit (OR-FFL) enables continuous production of Z even if X concentration drops below a threshold for a while.
